...
// Bakery tours & cocktail classes — Viator
export function getViatorUrl(city, country) {
  const query = `bakery tour cocktail ${city}`;
  if (VIATOR_PID) {
...
// Bakery tours & cocktail experiences — GetYourGuide
export function getGetYourGuideUrl(city, country) {
  const query = `bakery tour cocktail class ${city}`;
  if (GYG_PARTNER_ID) {
