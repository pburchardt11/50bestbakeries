...
// JSON data files — imported as default exports by the bundler
import _ENRICHED_RAW from './enriched-index.json'; // Google Places enrichment (ratings, photos, coords)
import _EDITORIAL_LOOKUP_RAW from './editorial-lookup.json';
// Resolve potential .default wrapper from JSON imports
const ENRICHED = _ENRICHED_RAW?.default || _ENRICHED_RAW || {};
const EDITORIAL_LOOKUP = _EDITORIAL_LOOKUP_RAW?.default || _EDITORIAL_LOOKUP_RAW || {};
 // spa-review-style scoring + badges
...
// Photos are now in a PUBLIC Vercel Blob store — URLs are direct, no proxy needed.
function proxyBlobUrl(blobUrl) { return blobUrl; }
