...
function expandBar(entry, city, country, rank = 0) {
  ...
  // Merge in Google Places enrichment when available (real ratings, photos, address)
  return {
    name: fullName,
    slug,
    city,
    country,
    rating: enriched?.rating ?? (ratingX10 / 10),
    reviews: enriched?.reviewCount ?? (reviewsDiv10 * 10),
    type,
    typeColor: TYPE_COLORS[type] || '#c2884a',
    typeIcon: TYPE_ICONS[type] || '🥐',
    rank,
    photo: enriched?.photoUrls?.[0] || getBarPhotoUrl(fullName, city),
    ...
    photoUrls: enriched?.photoUrls || null,
    isEnriched: !!enriched,
  };
}
...
function editorialToBar(entry) {
  ...
  return {
    ...
    photo: enriched?.photoUrls?.[0] || entry.images?.[0] || getBarPhotoUrl(entry.name, entry.city),
    address: enriched?.formattedAddress || null,
    phone: enriched?.phone || null,
    website: enriched?.website || null,
...
export function getGlobalTop() {
  return G50.map((entry, i) => ({ ...g50ToBar(entry), rank: i + 1 }));
}
...
export function getBarsForCountry(country, limit = 50) {
  ...
  return unique
    .sort((a, b) =>
      ...
      // 2. Editorial second — sorted by editorial sourceRank (curator/wikidata rank), lower is better
      (b.isEditorial === a.isEditorial ? 0 : b.isEditorial ? 1 : -1) ||
      (a.isEditorial && b.isEditorial ? (a.sourceRank || 999) - (b.sourceRank || 999) : 0) ||
      // 3. Everything else by rating then reviews
