...
function expandBar(entry, city, country, rank = 0) {
  ...
  return {
    ...
    rank,
    photo: getBarPhotoUrl(fullName, city),
  };
}
...
function editorialToBar(entry) {
  ...
  return {
    name: entry.name,
    slug: barSlug(entry.name, entry.city),
    city: entry.city,
    country: entry.country,
    rating: ratingX10 / 10,
    reviews: reviewsDiv10 * 10,
    ...
    photo: entry.images?.[0] || getBarPhotoUrl(entry.name, entry.city),
    tag: null,
    aw,
